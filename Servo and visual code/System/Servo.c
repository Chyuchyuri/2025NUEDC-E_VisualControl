...
{
	Angle1 = (Angle1 > MAX_ANGLE_X) ? MAX_ANGLE_X : ((Angle1 < MIN_ANGLE_X) ? MIN_ANGLE_X : Angle1);   //舵机限幅
	PWM_SetCompare2(Angle1 / 180 * 2000 + 500);
	
}
void Servo_SetAngle2(float Angle2)
{
	Angle2 = (Angle2 > MAX_ANGLE_Y) ? MAX_ANGLE_Y : ((Angle2 < MIN_ANGLE_Y) ? MIN_ANGLE_Y : Angle2);   //舵机限幅
